...
import { createMemorySharedCache, type SharedCache } from "../src/application/shared-cache.ts";
import { readOAuthAppConfig, missingOAuthConfig, oauthRedirectUri } from "../src/platform/zhihu/oauth.ts";
...
import { createKvSharedCache, readRedisRestConfig } from "../src/storage/kv-shared-cache.ts";
import { createSharedCacheHomeFeedCacheStore } from "../src/storage/home-feed-cache.ts";
import { createSharedCachePersonalArchiveStore } from "../src/storage/personal-archive-store.ts";
import {
  createKvRateLimiter,
...
// 生产必须配置 KV：Serverless 实例之间不共享内存，否则登录会话会随机丢失。
