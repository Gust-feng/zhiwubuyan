...
**你要做的是两件事**：
(1) 在 <main class="stage"> 里画你的图示与文字；
...
const RECIPES = `## 五、常用动画配方（可直接照抄改写）

**并排对比**：左右两个等宽面板，各自写标题，面板里放几行文字或小方块。
...
export function buildConceptAnimationPrompt(input: ConceptAnimationPromptInput): ConceptAnimationPrompt {
  ...
  lines.push("请直接输出完整的 HTML 文档（骨架已给出，替换占位符并填好图示与 steps）。");
